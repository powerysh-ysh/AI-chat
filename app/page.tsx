...
const steps = ["팀 등록", "M1 문제", "M2 해결책", "AI 사업화", "사업 한 장", "발표 준비"];
...
export default function Home() {
  ...
  useEffect(() => {
    localStorage.setItem("local-hero-project", JSON.stringify({ form, result, selectedName, projectCode }));
  }, [form, result, selectedName, projectCode]);
    ...
    const timer = window.setTimeout(async () => {
      try {
        const response = await fetch("/api/projects", {
          ...
          body: JSON.stringify({ code: projectCode, form, result, selectedName, step }),
        });
  ...
  }, [started, projectCode, form, result, selectedName, step]);
  ...
  const progress = Math.round(((result ? Math.max(step, 4) : step) / (steps.length - 1)) * 100);
...
${result.pitch}`;
  ...
  function reset() {
    ...
    setForm(empty); setResult(null); setSelectedName(""); setProjectCode(""); setStarted(false); setStep(0);
  ...
  async function restoreProject() {
    ...
    try {
      ...
      setStep(data.project.result ? Math.max(4, data.project.step) : data.project.step);
      setStarted(true);
    } catch (e) {
      ...
      {!started ? (
        <section className="hero shell">
          <div className="hero-copy">
            ...
            {result && <button className="resume" onClick={() => { setStarted(true); setStep(4); }}>저장된 결과 이어보기</button>}
          ...
          <div className="mission-rail">{steps.map((x,i)=><div key={x}><b>{i+1}</b><span>{["👋","🔎","💡","🤖","📄","📣"][i]}</span><strong>{x}</strong></div>)}</div>
        </section>
      ) : (
        <section className="workspace shell">
          ...
          <div className="step-dots">{steps.map((x,i)=><button key={x} className={i===step?"on":i<step?"done":""} disabled={i>2 && !result} onClick={()=>setStep(i)}><b>{i < step || result && i < 4 ? "✓" : i+1}</b><span>{x}</span></button>)}</div>
          ...
          {step === 1 && <MissionCard icon="🔎" coach="M1에서 완성한 ‘문제 한 문장’을 그대로 옮겨 적으세요. 누가, 어떤 상황에서, 왜 불편한지가 보이면 좋아요.">
            ...
            <Nav onBack={()=>setStep(0)} disabled={!form.problem.trim()} onNext={()=>setStep(2)} />
          </MissionCard>}

          {step === 2 && <MissionCard icon="💡" coach="M2에서 팀이 투표로 선택한 해결 문장을 입력하세요. 완벽하지 않아도 괜찮아요. AI 코치가 함께 다듬어 줍니다.">
            <label className="field-label">M2. 우리가 선택한 해결 아이디어</label>
            <textarea className="big-input" value={form.solution} onChange={e=>setForm({...form,solution:e.target.value})} placeholder="예: 우리 팀은 외국인 관광객을 위해 주문의 어려움을 해결하는 다국어 시장 음식 추천 챗봇을 제안합니다."/>
            <div className="formula">💡 <strong>문장 공식</strong>　우리 팀은 [고객]을 위해 [문제]를 해결하는 [서비스]를 제안합니다.</div>
            ...
            <Nav onBack={()=>setStep(1)} disabled={!form.solution.trim()} onNext={createBusiness} nextLabel="AI로 사업화하기 ✨" />
          </MissionCard>}

          {step === 3 && <div className="loading-card">
            <div className="loader">🤖</div><h2>AI 창업 코치가 회의 중입니다</h2>
            <p>고객을 찾고 · 차별점을 만들고 · 발표문을 쓰는 중</p><div className="loading-line"><i/></div>
            {error && <><p className="error">{error}</p><button className="primary" onClick={createBusiness}>다시 시도</button></>}
          </div>}

          {step === 4 && result && <div className="result-grid">
            ...
            <aside className="tool-card">
              ...
              <button className="primary full" onClick={()=>setStep(5)}>3분 발표 준비 →</button>
              ...
              <button className="text-button" onClick={()=>setStep(2)}>← 입력 내용 다시 보기</button>
            </aside>
          </div>}
          ...
          {step === 5 && result && <div className="result-grid">
            ...
            <aside className="qa-card">
              ...
              <button className="back full" onClick={()=>setStep(4)}>← 사업 한 장 수정</button>
            </aside>
          </div>}
        </section>
      )}
